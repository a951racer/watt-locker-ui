...
export default function MonthlyDistanceChart({ workouts }: MonthlyDistanceChartProps) {
  const data = useMemo(() => {
    const now = new Date();
    ...
    return months.map(({ label, year, month }) => {
      const total = workouts
        ...
        .reduce((sum, w) => sum + (Number(w.distanceMeters) || 0), 0);
      const miles = Math.round(total / 1609.344);
      return { month: label, miles };
    });
  }, [workouts]);
  ...
  return (
    <div className="bg-midnightBlue/80 rounded-xl p-4 border border-steelBlue/50">
      <h3 className="text-sm font-semibold text-softFog uppercase tracking-wide mb-3">
        Total Distance per Month (6 Months)
      </h3>
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
          ...
          <YAxis tick={{ fill: '#7E93AD', fontSize: 11 }} axisLine={{ stroke: '#2E4767' }} tickLine={false} width={45} />
          <Tooltip
            contentStyle={{ backgroundColor: '#0D2A4F', border: '1px solid #2E4767', borderRadius: '8px', color: '#D9E1EA' }}
            formatter={(value) => [`${Number(value).toLocaleString()} mi`, 'Distance']}
          />
          <Bar dataKey="miles" fill="#1E7EF2" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
