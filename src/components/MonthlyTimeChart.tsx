...
export default function MonthlyTimeChart({ workouts }: MonthlyTimeChartProps) {
  const data = useMemo(() => {
    const now = new Date();
    ...
    return months.map(({ label, year, month }) => {
      const total = workouts
        ...
        .reduce((sum, w) => sum + (Number(w.movingTimeSeconds ?? w.durationSeconds) || 0), 0);
    ...
    <div className="bg-midnightBlue/80 rounded-xl p-4 border border-steelBlue/50">
      ...
      <ResponsiveContainer width="100%" height={270}>
        <BarChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
          ...
          <YAxis tick={{ fill: '#7E93AD', fontSize: 11 }} axisLine={{ stroke: '#2E4767' }} tickLine={false} width={40} />
          <Tooltip
            contentStyle={{ backgroundColor: '#0D2A4F', border: '1px solid #2E4767', borderRadius: '8px', color: '#D9E1EA' }}
            formatter={(value) => [`${value} hrs`, 'Total Time']}
          />
          <Bar dataKey="hours" fill="#3FA9FF" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
